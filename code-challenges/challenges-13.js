...
const objectCounter = (obj) => {
  ...
  //  }, 0);
  //  return count;  
  const count = obj.reduce(function (acc, obj) {
    acc += 1;
    return acc;
  }, 0);
  return count;
...
const stringReverse = (str) => {
  ...
  const reversedStr = str.split(' ').reduceRight(function (acc, word) {
    return acc + ' ' + word;
  }, '').trim();
...
const statistics = (obj) => {
  ...
  let candidateVotes = obj.reduce((acc, voter) => {
    let candidate = voter.votes_To;
    if (acc[candidate]) {
      acc[candidate]++;
    } else {
      acc[candidate] = 1;
