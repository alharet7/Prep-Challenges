...
const cvsFiltration = (arr) => {
    ...
    const result = arr.filter(emp => emp.tech == "JS" && emp.yearsOfExperience > 4).map(emp => {

        const fullName = (emp.firstName && emp.LastName) ? emp.firstName + " " + emp.LastName : emp.firstName || emp.LastName;
        return {
            fullName: fullName,
            tech: emp.tech
        };
